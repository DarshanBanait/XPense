...
data_entry_frame_bg = '#F0F0F0'
buttons_frame_bg = '#E0E0E0'
hlb_btn_bg = '#854E1E'

lbl_font = ('Helvetica', 13)
entry_font = ('Helvetica', 13, 'bold')
...
Label(root, text='XPense', font=('Noto Sans CJK TC', 15, 'bold'), bg=hlb_btn_bg).pack(side=TOP, fill=X)
...
# Data Entry Frame
Label(data_entry_frame, text='Date (M/DD/YY) :', font=lbl_font, bg=data_entry_frame_bg).place(x=10, y=50)
date = DateEntry(data_entry_frame, date=datetime.datetime.now().date(), font=entry_font)
date.place(x=160, y=50)

Label(data_entry_frame, text='Payee\t             :', font=lbl_font, bg=data_entry_frame_bg).place(x=10, y=230)
Entry(data_entry_frame, font=entry_font, width=31, text=payee).place(x=10, y=260)

Label(data_entry_frame, text='Description           :', font=lbl_font, bg=data_entry_frame_bg).place(x=10, y=100)
Entry(data_entry_frame, font=entry_font, width=31, text=desc).place(x=10, y=130)

Label(data_entry_frame, text='Amount\t             :', font=lbl_font, bg=data_entry_frame_bg).place(x=10, y=180)
Entry(data_entry_frame, font=entry_font, width=14, text=amnt).place(x=160, y=180)

Label(data_entry_frame, text='Mode of Payment:', font=lbl_font, bg=data_entry_frame_bg).place(x=10, y=310)
...
Button(data_entry_frame, text='Add Expense', command=add_another_expense, font=btn_font, width=30,
       bg=hlb_btn_bg, borderwidth=2, relief="raised").place(x=10, y=395)
